...
function TicketInformation({ isUpcoming }) {
  ...
  useEffect(() => {
    ...
    if (isUpcoming === true) getTickets("flights/getUpcomingFlights");
    else getTickets("flights/getUpcomingFlights");
  }, [currentUser, isUpcoming]);

  const cancelTicket = () => {
    axios
      .post(`flights/cancelFlight/${toDeleteTicketId}`)
      .then((res) => {
        // console.log("res");
        console.log(res);
        setToDeleteTicketId("");
        ...
        getTickets("flights/getUpcomingFlights");
      })
      .catch((err) => {
        // console.log("err");
        console.log(err);
                  ...
                  >
                    ...
                    {tickets.map((ticket, idx) => (
                      <Ticket
                        ...
                        flightId={ticket.flight_code}
                        srcId={ticket.srcIATA_code}
                        srcAirportName={ticket.src_airport}
                        srcCity={ticket.source}
                        srcCountry={ticket.src_country_name}
                        // srcOffset={offset.get(ticket.SRC_AIRPORT_ID)}
                        destId={ticket.dstIATA_code}
                        destAirportName={ticket.dst_airport}
                        destCity={ticket.destination}
                        destCountry={ticket.dst_country_name}
