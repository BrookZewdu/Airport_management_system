...
const Booking = () => {
  ...
  const addPassenger = (row, col) => {
    ...
    setPassengerDetails([curr[curr.length - 1]]);
  };
  ...
  const handleSeatClick = (row, col) => {
    ...
    if (seat) {
      localSeatData[seat.row][seat.col] = 0;
      setSeat({ row, col });
      localSeatData[row][col] = 1;
      setCost(location?.state?.fare);
      console.log("+++++++++++++++++++", cost);
  ...
  const bookTickets = () => {
    ...
    const seat_number =
      (passengerDetails[0].row - 1) * 4 + passengerDetails[0].col;
    for (let i = 0; i < payloadContent.length; i++) {
      payloadContent[i] = {
        flight_code: location?.state?.flightId,
        source: location?.state?.srcCity,
        destination: location?.state?.destCity,
        price: passengerDetails[i].fare,
        seat_number,
        date_of_travel: location?.state?.departureDate,
      };
    }
    axios
      .post(`flights/bookFlight/`, payloadContent[payloadContent.length - 1], {
        Headers: {
  ...
  useEffect(() => {
    ...
    axios
      .get(`flights/seatsTaken/${location?.state?.flightId}`)
      .then((res) => {
        let localSeatData = new Array(location?.state?.numRows);
        // let localSeatData = new Array(24);
        for (let i = 0; i < localSeatData.length; i++)
          localSeatData[i] = new Array(location?.state?.numCols);
        // localSeatData[i] = new Array(4);
        for (let i = 0; i < localSeatData.length; i++) {
        ...
        res.data.data.forEach((ticket) => {
          let ROW_NUM = Math.floor((ticket - 1) / 4);
          let COL_NUM = (ticket - 1) % 4;
          localSeatData[ROW_NUM][COL_NUM] = 2;
        });
              ...
              <Card sx={{ backgroundColor: "#FFFFFF" }}>
                ...
                >
                  <Header
                    ...
                    destCity={location?.state?.destination}
