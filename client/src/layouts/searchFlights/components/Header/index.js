...
function Header({ children, airports }) {
  ...
  //     offsetData.set(airport.AIRPORT_ID, 0);
  //   });
  ...
  const handleSearch = () => {
    ...
    axios
      .get(
        `flights/searchFlights/${source.state}/${destination.state}/${moment(
          dateValue
        ).format()}`
      )
      .then((res) => {
        ...
        res.data.data.forEach((flight) => {
          let hr = parseInt(flight.duration / 60) + "";
          let min = parseInt(flight.duration % 60) + "";
  ...
  const cancelFlight = (flightDateId) => {
    ...
    axios
      .post(`flights/cancelFlight/${currFlight.FLIGHT_ID}`, {
        CANCEL: true,
      ...
      >
        ...
        {loading ? (
          ...
          <TicketInformation
            ...
            srcCity={`${source?.state}`}
            srcID={source?.IATA_code}
            destID={destination?.IATA_code}
